...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows > 0){
        ...
        echo '<script> alert("Идентификатор неизвестной недвижимости."); location.replace("./?page=real_estate"); </script>';
    }
}else{
    echo '<script> alert("Идентификатор недвижимости требуется для доступа к странице."); location.replace("./?page=real_estate"); </script>';
}
?>
<style>
    .view-image img{
...
<div class="content py-3">
    <div class="card card-outline card-primary rounded-0 shadow">
        <div class="card-header">
            <h4 class="card-title"> Детали недвижимости ID №_: <b><?= isset($code) ? $code : "" ?></b></h4>
            <div class="card-tools">
                <a href="./?page=real_estate/manage_estate&id=<?= isset($id) ? $id : '' ?>" class="btn btn-flat btn-sm bg-gradient-primary" style="border-radius: 5px;"><i class="fa fa-edit"></i> Редактировать детали недвижимости </a>
                <button type="button" class="btn btn-flat btn-sm bg-gradient-danger" id="delete_estate" style="border-radius: 5px;"><i class="fa fa-trash"></i> Удалить детали недвижимости </button>
            </div>
        </div>
        <div class="card-body">
            <div class="row gx-4 gx-lg-5 align-items-top">
                ...
                <div class="col-md-6">
                    ...
                    <fieldset>
                        <legend class="h4 text-muted"> Подробнее: </legend>
                        <div class="row">
                            <div class="col-6">
                                <span class="text-muted"> Тип: </span><?= isset($type) ? $type : '' ?>
                            </div>
                            <div class="col-6">
                                <span class="text-muted"> Цель: </span><?= isset($purpose) ? $purpose : '' ?>
                            </div>
                            <div class="col-6">
                                <span class="text-muted"> Площадь: </span><?= isset($area) ? $area : '' ?>
                            </div>
                            <div class="col-6">
                                <span class="text-muted"> Цена продажи: </span><?= isset($sale_price) ? format_num($sale_price) : '' ?>
                            </div>
                            <div class="col-6">
                                <span class="text-muted"> Местоположение: </span><?= isset($location) ? $location : '' ?>
                            </div>
                            <div class="col-6">
                                <span class="text-muted"> Состояние: </span> 
                                <?php if(isset($status) && $status == 1): ?>
                                    <span class="badge badge-success px-3 rounded-pill"> Доступно </span>
                                <?php else: ?>
                                    <span class="badge badge-danger px-3 rounded-pill"> Недоступно </span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset>
                        ...
                        <?php  if(isset($amenity_arr[1]) && count($amenity_arr[1]) > 0): ?>
                            ...
                            <div><b> На открытом воздухе: </b></div>
                            <div class="row">
                        ...
                        <?php endif; ?>
                        <?php else: ?>
                            <center><small class="text-mute"><i> Нет перечисленных удобств. </i></small></center>
                        <?php endif; ?>
                    </fieldset>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-lg-6 col-md-8 col-sm-12 col-xs-12">
                    <div class="card card-outline card-info rounded-0 shadow">
                        <div class="card-header">
                            <h4 class="cart-title"><b> Подробности об агента: </b></h4>
                        </div>
                        <div class="card-body">
                            <div class="row">
...
$(function(){
    ...
    $('#delete_estate').click(function(){
        _conf("Вы уверены, что хотите удалить эту недвижимость навсегда?","delete_estate",[])
    })
})
